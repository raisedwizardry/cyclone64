...
ParticleSpring::ParticleSpring(Particle *other, float sc, float rl)
...
ParticleBuoyancy::ParticleBuoyancy(float maxDepth,
                                 float volume,
                                 float waterHeight,
                                 float liquidDensity)
:
...
ParticleBungee::ParticleBungee(Particle *other, float sc, float rl)
: other(other), springConstant(sc), restLength(rl)
{
}
...
ParticleFakeSpring::ParticleFakeSpring(Vector3 *anchor, float sc, float d)
...
void ParticleFakeSpring::updateForce(Particle* particle, float duration)
{
    ...
    float gamma = 0.5f * real_sqrt(4 * springConstant - damping*damping);
    ...
    Vector3 accel = (target - position) * ((float)1.0 / (duration*duration)) -
        particle->getVelocity() * ((float)1.0/duration);
...
void ParticleAnchoredSpring::init(Vector3 *anchor, float springConstant,
                                  float restLength)
{
